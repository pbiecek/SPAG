...
* **eachArea** - by default the SPAG index is calculated for the whole area. This can be changed by setting this parameter to true - if so the index is calculated for each area within the spatial data frame separately.


```{r, message=FALSE, warning=FALSE}
# ?SPAG
SPAGIndex <- SPAG(companiesDF = CompaniesPoland, shp = ShapefilePoland, theoreticalSample=100, empiricalSample=200)
print(SPAGIndex)
```
...
Parameter numberOfSamples defines the number of times the distance index will be calculated. This increases the total time of calculations but results in more accurate result
```{r, message=FALSE, warning=FALSE}
startTime <- Sys.time()
SPAGIndexSAM <- SPAG(companiesDF = CompaniesPoland, shp = ShapefilePoland, theoreticalSample=100, empiricalSample=100,
                     numberOfSamples=1)
...
```{r, message=FALSE, warning=FALSE}
SPAGIndex <- SPAG(companiesDF = CompaniesPoland, shp = ShapefilePoland, theoreticalSample=100, empiricalSample=100,
                     numberOfSamples=1, eachArea = TRUE)
SPAGIndex
```
## Plotting the Index
...
```{r}
SPAGIndex <- SPAG(companiesDF = CompaniesPoland, shp = ShapefilePoland, CRSProjection="+proj=longlat +datum=WGS84")
ggplot(SPAGIndex)
```